...
from sqlalchemy import CheckConstraint, ForeignKey, ForeignKeyConstraint, String, Integer
...
class Subject(Base):
    ...
    sex: Mapped[Optional[str]] = mapped_column(String(100), CheckConstraint("sex IN ('M', 'F', 'Other')"))


class Sample(Base):
    __tablename__ = "sample"
    sample_id: Mapped[str] = mapped_column(String(100), primary_key=True)
    sample_type: Mapped[Optional[str]] = mapped_column(String(100), CheckConstraint("sample_type IN ('PBMC', 'WB')"))
    ...
    subject_id: Mapped[str] = mapped_column(ForeignKey("subject.subject_id"), nullable=False)
